...
Runs its own engine and Docker runtime over stdin/stdout. Logs go to stderr. It does not connect to `serve`; stop the other runtime before reusing its resources. See [MCP setup](mcp.md). There is no `mcp install` command.
